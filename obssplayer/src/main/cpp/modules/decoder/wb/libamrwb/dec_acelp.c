...
extern const Word16t D_ROM_ph_imp_low[];
extern const Word16t D_ROM_ph_imp_mid[];
...
void D_ACELP_decode_2t(Word16t index, Word16t code[])
{
   ...
   memset(code, 0, 64 * sizeof(Word16t));
                              ...
                              Word16t mode, Word16t disp_mem[])
{
   ...
   Word32 i, j, state;
   Word16t *prev_gain_pit, *prev_gain_code, *prev_state;
   ...
   *prev_state = (Word16t)state;
   ...
   if(state < 2)
   {
      for(i = 0; i < L_SUBFR; i++)
      {
         code[i] = (Word16t)(code2[i] + code2[i + L_SUBFR]);
      }
   }
