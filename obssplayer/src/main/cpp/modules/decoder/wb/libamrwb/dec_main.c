...
//#define MAX_16 (Word16t)0x7fff
//#define MIN_16 (Word16t)0x8000
...
extern const Word16t D_ROM_isp[];
extern const Word16t D_ROM_isf[];
extern const Word16t D_ROM_interpol_frac[];
...
void D_MAIN_reset(void *st, Word16t reset_all)
{
   ...
   memset(dec_state->mem_exc, 0, (PIT_MAX + L_INTERPOL) * sizeof(Word16t));
   memset(dec_state->mem_isf_q, 0, M * sizeof(Word16t));
   dec_state->mem_T0_frac = 0;   /* old pitch value = 64.0 */
   dec_state->mem_T0 = 64;
   dec_state->mem_first_frame = 1;
   dec_state->mem_gc_thres = 0;
   dec_state->mem_tilt_code = 0;
   memset(dec_state->mem_ph_disp, 0, 8 * sizeof(Word16t));
   ...
   if(reset_all != 0)
   {
      ...
      memset(dec_state->mem_oversamp, 0, (2 * 12) * sizeof(Word16t));
      memset(dec_state->mem_sig_out, 0, 6 * sizeof(Word16t));
      memset(dec_state->mem_hf, 0, (31 - 1) * sizeof(Word16t));
      memset(dec_state->mem_hf3, 0, (31 - 1) * sizeof(Word16t));
      memset(dec_state->mem_hp400, 0, 6 * sizeof(Word16t));
      D_GAIN_lag_concealment_init(dec_state->mem_lag);

      /* isp initialization */
      memcpy(dec_state->mem_isp, D_ROM_isp, M * sizeof(Word16t));
      memcpy(dec_state->mem_isf, D_ROM_isf, M * sizeof(Word16t));

      for(i = 0; i < L_MEANBUF; i++)
      {
         memcpy(&dec_state->mem_isf_buf[i * M], D_ROM_isf, M * sizeof(Word16t));
      }
      ...
      memset(dec_state->mem_syn_hf, 0, M16k * sizeof(Word16t));
      memset(dec_state->mem_syn_hi, 0, M * sizeof(Word16t));
      memset(dec_state->mem_syn_lo, 0, M * sizeof(Word16t));
                     ...
                     void *spd_state, UWord8 frame_type)
{
   ...
   Word16t exc2[L_FRAME];            /* excitation vector                   */
   Word16t Aq[NB_SUBFR * (M + 1)];   /* A(z) quantized for the 4 subframes  */
   Word16t code[L_SUBFR];            /* algebraic codevector                */
   Word16t excp[L_SUBFR];            /* excitation vector                   */
   Word16t HfIsf[M16k];
   Word16t ispnew[M];                /* immittance spectral pairs at 4nd sfr*/
   Word16t isf[M];                   /* ISF (frequency domain) at 4nd sfr   */
   Word16t isf_tmp[M];               /* ISF tmp                             */
   Word16t ind[8];                   /* quantization indices                */

   Word16t index, fac, voice_fac, max, Q_new = 0;
   Word16t gain_pit, gain_code, gain_code_lo, tmp;
   Word16t corr_gain = 0;
   //was: UWord16 pit_sharp = 0;
   Word32 pit_sharp = 0;  // deng

   Word16t *exc;                     /* Excitation vector                   */
   Word16t *p_Aq;                    /* ptr to A(z) for the 4 subframes     */
   Word16t *p_isf;                   /* prt to isf                          */
   ...
   if(newDTXState == SPEECH)
   {
      ...
      if(bfi == 0)
      {
         ...
         {
			/* was:
            st->mem_vad_hist = (Word16t)(st->mem_vad_hist + 1);
   ...
   if(newDTXState != SPEECH) /* CNG mode */
   {
      ...
      memcpy(isf_tmp, st->mem_isf, M * sizeof(Word16t));
      ...
      for(i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
      {
         ...
         for(i = 0; i < M; i++)
         {
            ...
            HfIsf[i] = (Word16t)((L_tmp + 0x8000) >> 16);
         }

         D_UTIL_dec_synthesis(Aq, &exc2[i_subfr], 0, &synth16k[i_subfr * 5 /4],
            (Word16t) 1, HfIsf, mode, newDTXState, bfi, st);
      }

      /* reset speech coder memories */
      D_MAIN_reset(st, 0);
      memcpy(st->mem_isf, isf, M * sizeof(Word16t));
      st->mem_bfi = bfi;
   ...
   if(st->mem_first_frame != 0)
   {
      ...
      memcpy(st->mem_isp, ispnew, M * sizeof(Word16t));
   }
   ...
   for(i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
   {
      ...
      if(pit_flag == 0)
      {
         ...
         else
         {
            ...
            {
               index = (Word16t)((index - ((PIT_FR2 - PIT_MIN) * 4)));
               T0 = PIT_FR2 + (index >> 1);
      ...
      if(select == 0)
      {
         ...
         for(i = 0; i < L_SUBFR; i++)
         {
            ...
            code[i] = (Word16t)((L_tmp + 0x2000) >> 14);
         }

         memcpy(&exc[i_subfr], code, L_SUBFR * sizeof(Word16t));
      }
      ...
      if(unusable_frame != 0)
      {
         ...
         for(i = 0; i < L_SUBFR; i++)
         {
            code[i] = (Word16t)(D_UTIL_random(&(st->mem_seed)) >> 3);
         }
      }
      ...
      {
         memcpy(ind, prms, 4 * sizeof(Word16t));
         prms += 4;
      ...
      {
         memcpy(ind, prms, 8 * sizeof(Word16t));
         prms += 8;
      ...
      {
         L_tmp = (L_tmp << 1);
         Q_new = (Word16t)((Q_new + 1));
      }

      if(L_tmp < 0x7FFF7FFF)
      {
         gain_code = (Word16t)((L_tmp + 0x8000) >> 16);
      ...
      if(Q_new > st->mem_q)
      {
         D_UTIL_signal_up_scale(exc + i_subfr - (PIT_MAX + L_INTERPOL),
            PIT_MAX + L_INTERPOL + L_SUBFR, (Word16t)(Q_new - st->mem_q));
      }
      else
      {
         D_UTIL_signal_down_scale(exc + i_subfr - (PIT_MAX + L_INTERPOL),
            PIT_MAX + L_INTERPOL + L_SUBFR, (Word16t)(st->mem_q - Q_new));
      }
      ...
      if(bfi == 0)
      {
         ...
         st->mem_lag[0] = (Word16t)T0;
         st->mem_T0 = (Word16t)T0;
         st->mem_T0_frac = 0;   /* Remove fraction in case of BFI */
      }

      /* find voice factor in Q15 (1=voiced, -1=unvoiced) */
      memcpy(exc2, &exc[i_subfr], L_SUBFR * sizeof(Word16t));
      D_UTIL_signal_down_scale(exc2, L_SUBFR, 3);

      /* post processing of excitation elements */
      if(mode <= MODE_9k)
      {
         pit_sharp = (Word16t)(gain_pit << 1);
         ...
         if(pit_sharp > 16384)
         {
            ...
            for(i = 0; i < L_SUBFR; i++)
            {
               ...
               excp[i] = (Word16t)((L_tmp + 0x8000) >> 16);
            }
         }
      }

      voice_fac = D_GAIN_find_voice_factor(exc2, -3, gain_pit, code, gain_code, L_SUBFR);

      /* tilt of code for next subframe: 0.5=voiced, 0=unvoiced */
      st->mem_tilt_code = (Word16t)((voice_fac >> 2) + 8192);
      ...
      for(i = 0; i < L_SUBFR; i++)
      {
         ...
         {
            exc[i + i_subfr] = (Word16t)L_tmp;
            tmp = (Word16t)(abs(L_tmp));
      ...
      tmp = (Word16t)((D_UTIL_norm_s(max) + Q_new) - 1);
      ...
      D_ACELP_phase_dispersion(gain_code, gain_pit, code, (Word16t)j,
         st->mem_ph_disp);
      ...
      fac = (Word16t)((L_stab_fac * L_tmp) >> 15);
      L_tmp = L_gain_code;
      ...
      L_gain_code =
         D_UTIL_mpy_32_16(gain_code, gain_code_lo, (Word16t)(32767 - fac));
      ...
      /* build excitation */
      gain_code = (Word16t)(((L_gain_code << Q_new) + 0x8000) >> 16);
      ...
      if(mode <= MODE_9k)
      {
         if(pit_sharp > 16384)
         {
            ...
            memcpy(exc2, excp, L_SUBFR * sizeof(Word16t));
         }
      }
      ...
      if(mode <= MODE_7k)
      {
         ...
         for(i = 0; i < M; i++)
         {
            ...
            HfIsf[i] = (Word16t)((L_tmp + 0x4000) >> 15);
         }
      }
      else
      {
         memset(st->mem_syn_hf, 0, (M16k - M) * sizeof(Word16t));
      }
   ...
   memmove(st->mem_exc, &st->mem_exc[L_FRAME], (PIT_MAX + L_INTERPOL) * sizeof(Word16t));
