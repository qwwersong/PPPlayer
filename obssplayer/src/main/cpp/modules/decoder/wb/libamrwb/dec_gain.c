...
extern const Word16t D_ROM_inter4_2[];
extern const Word16t D_ROM_inter4_2_X[];  // deng

extern const Word16t D_ROM_pdown_unusable[];
extern const Word16t D_ROM_pdown_usable[];
extern const Word16t D_ROM_cdown_unusable[];
extern const Word16t D_ROM_cdown_usable[];
extern const Word16t D_ROM_qua_gain6b[];
extern const Word16t D_ROM_qua_gain7b[];
...
void D_GAIN_init(Word16t *mem)
{
    ...
    * mem[17 - 21] = 0; pbuf2[i]
    */
   memset(&mem[4], 0, 18 * sizeof(Word16t));
 ...
 */
static Word16t D_GAIN_median(Word16t x[])
{
   Word16t x1, x2, x3, x4, x5;
   Word16t tmp;
   x1 = x[ - 2];
   x2 = x[ - 1];
   x3 = x[0];
                   ...
                   Word16t *mem)
{
   ...
   const Word16t * p;
   Word16t *past_gain_pit, *past_gain_code, *past_qua_en, *prev_gc;
   Word16t *gbuf, *pbuf, *pbuf2;
   Word16t i, tmp, exp, frac, exp_gcode0, gcode_inov;
   Word16t g_code;
   ...
   exp = (Word16t)(exp - (18 + 6));   /* exp: -18 (code in Q9), -6 (/L_subfr) */
   ...
   gcode_inov = (Word16t)(L_tmp >>16);   /* g_code_inov in Q12 */
   ...
   if(bfi != 0)
   {
      ...
      *past_gain_pit = tmp;
      ...
      {
         *gain_pit =
            (Word16t)((D_ROM_pdown_unusable[state] * *past_gain_pit) >> 15);
      }
      else
      {
         *gain_pit =
            (Word16t)((D_ROM_pdown_usable[state] * *past_gain_pit) >> 15);
      }
      ...
      else
      {
         ...
         {
            *past_gain_code =
               (Word16t)((D_ROM_cdown_unusable[state] * tmp) >> 15);
         }
         else
         {
            *past_gain_code =
               (Word16t)((D_ROM_cdown_usable[state] * tmp) >> 15);
         }
      }
      ...
      past_qua_en[0] = (Word16t)qua_ener;
    ...
    * 16384 < Pow2() <= 32767
    */
   exp_gcode0 = (Word16t)(exp_gcode0 - 14);
 ...
 *    void
 */
static void D_GAIN_insert_lag(Word16t array[], Word32 n, Word16t x)
{
   Word32 i;
 ...
 *    void
 */
static void D_GAIN_sort_lag(Word16t array[], Word16t n)
{
   Word32 i;
 ...
 *    void
 */
void D_GAIN_lag_concealment(Word16t gain_hist[], Word16t lag_hist[],
                            Word32 *T0, Word16t *old_T0, Word16t *seed,
                            Word16t unusable_frame)
{
   Word32 i, lagDif, tmp, tmp2, D2, meanLag = 0;
   Word16t lag_hist2[L_LTPHIST] = {0};
   Word16t maxLag, minLag, lastLag;
   Word16t minGain, lastGain, secLastGain;
   Word16t D;
 ...
 */
Word16t D_GAIN_find_voice_factor(Word16t exc[], Word16t Q_exc,
                                Word16t gain_pit, Word16t code[],
                                Word16t gain_code, Word16t L_subfr)
{

   Word32 tmp, ener1, ener2, i;
   Word16t exp, exp1, exp2;

   ener1 = (D_UTIL_dot_product12(exc, exc, L_subfr, &exp1)) >> 16;
   exp1 = (Word16t)(exp1 - (Q_exc + Q_exc));
   tmp = (gain_pit * gain_pit) << 1;
   exp = D_UTIL_norm_l(tmp);
   tmp = (tmp << exp) >> 16;
   ener1 = (ener1 * tmp) >> 15;
   exp1 = (Word16t)((exp1 - exp) - 10);   /* 10 -> gain_pit Q14 to Q9 */
   ener2 = D_UTIL_dot_product12(code, code, L_subfr, &exp2) >> 16;
   exp = D_UTIL_norm_s(gain_code);
   tmp = gain_code << exp;
   tmp = (tmp * tmp) >> 15;
   ener2 = (ener2 * tmp) >> 15;
   exp2 = (Word16t)(exp2 - (exp << 1));
   i = exp1 - exp2;
