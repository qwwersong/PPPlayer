...
extern const Word16t D_ROM_dico1_isf[];
extern const Word16t D_ROM_dico2_isf[];
extern const Word16t D_ROM_dico21_isf_36b[];
extern const Word16t D_ROM_dico22_isf_36b[];
extern const Word16t D_ROM_dico23_isf_36b[];
extern const Word16t D_ROM_dico21_isf[];
extern const Word16t D_ROM_dico22_isf[];
extern const Word16t D_ROM_dico23_isf[];
extern const Word16t D_ROM_dico24_isf[];
extern const Word16t D_ROM_dico25_isf[];
extern const Word16t D_ROM_dico1_isf_noise[];
extern const Word16t D_ROM_dico2_isf_noise[];
extern const Word16t D_ROM_dico3_isf_noise[];
extern const Word16t D_ROM_dico4_isf_noise[];
extern const Word16t D_ROM_dico5_isf_noise[];
extern const Word16t D_ROM_mean_isf[];
extern const Word16t D_ROM_mean_isf_noise[];
extern const Word16t D_ROM_cos[];
 ...
 *    void
 */
static void D_LPC_isf_reorder(Word16t *isf, Word16t min_dist, Word16t n)
{
   Word32 i, isf_min;
...
void D_LPC_isf_noise_d(Word16t *indice, Word16t *isf_q)
{
   ...
   for(i = 0; i < ORDER; i++)
   {
      isf_q[i] = (Word16t)(isf_q[i]+ D_ROM_mean_isf_noise[i]);
   }
...
void D_LPC_isf_isp_conversion(Word16t isf[], Word16t isp[], Word16t m)
{
   ...
   for(i = 0; i < m; i++)
   {
      ...
      isp[i] = (Word16t)(D_ROM_cos[ind] + (tmp >> 7));
   }
   return;
}
 ...
 *    void
 */
static void D_LPC_isp_pol_get(Word16t *isp, Word32 *f, Word32 n, Word16t k16)
{
   Word32 i, j, t0, s1, s2;
   Word16t hi, lo;
                            ...
                            Word16t m)
{
   ...
   for(i = 1, j = m - 1; i < nc; i++, j--)
   {
      ...
      tmax |= labs(t0);
      a[i] = (Word16t)((t0 + 0x800) >> 12);   /* from Q23 to Q12 and * 0.5 */
      
      /* a[j] = 0.5*(f1[i] - f2[i]); */
      t0 = (f1[i] - f2[i]);   /* f1[i] - f2[i] */
      tmax |= labs(t0);
      a[j] = (Word16t)((t0 + 0x800) >> 12);   /* from Q23 to Q12 and * 0.5 */
   }
      ...
      for (i = 1, j = m - 1; i < nc; i++, j--)
        {
          /* a[i] = 0.5*(f1[i] + f2[i]); */
          t0 = f1[i] + f2[i];          /* f1[i] + f2[i]             */
          a[i] = (Word16t)((t0 + r) >> q_sug); /* from Q23 to Q12 and * 0.5 */
          
          /* a[j] = 0.5*(f1[i] - f2[i]); */
          t0 = f1[i] - f2[i];          /* f1[i] - f2[i]             */
          a[j] = (Word16t)((t0 + r) >> q_sug); /* from Q23 to Q12 and * 0.5 */
        }
      a[0] = (Word16t)(a[0] >> q);             
    } 
    else 
   ...
   t0 = D_UTIL_mpy_32_16(hi, lo, isp[m - 1]);
   t0 = f1[nc] + t0;
   a[nc] = (Word16t)((t0 + r) >> q_sug);  /* from Q23 to Q12 and * 0.5 */

   /* a[m] = isp[m-1]; */
   a[m] = (Word16t)((isp[m - 1] >> (2 + q)) + 1); /* from Q15 to Q12 */
   a[m] = (Word16t)(a[m] >> 1);
...
void D_LPC_a_weight(Word16t a[], Word16t ap[], Word16t gamma, Word16t m)
{
   ...
   for(i = 1; i < m; i++)
   {
      ap[i] = (Word16t)(((a[i] * fac) + 0x4000) >> 15);
      fac = ((fac * gamma)  + 0x4000) >> 15;
   }

   ap[m] = (Word16t)(((a[m] * fac) + 0x4000) >> 15);
                           ...
                           Word16t *isfold, Word16t *isf_buf, Word16t bfi)
{
   ...
   if(bfi == 0) /* Good frame */
   {
      ...
      {
         isf_q[i] =
            (Word16t)(isf_q[i] + D_ROM_dico21_isf_36b[indice[2] * 5 + i]);
      }

      for(i = 0; i < 4; i++)
      {
         isf_q[i + 5] =
            (Word16t)(isf_q[i + 5] + D_ROM_dico22_isf_36b[indice[3] * 4 + i]);
      }

      for(i = 0; i < 7; i++)
      {
         isf_q[i + 9] =
            (Word16t)(isf_q[i + 9] + D_ROM_dico23_isf_36b[indice[4] * 7 + i]);
      }
      ...
      for(i = 0; i < ORDER; i++)
      {
         tmp = isf_q[i];
         isf_q[i] =
            (Word16t)((tmp + D_ROM_mean_isf[i]) + ((MU * past_isfq[i]) >> 15));
         past_isfq[i] = tmp;
      }
   ...
   {   /* bad frame */
      ...
      for(i = 0; i < ORDER; i++)
      {
         isf_q[i] = (Word16t)((((ALPHA * isfold[i]) >> 15) +
      ...
      for(i = 0; i < ORDER; i++)
      {
         ...
         /* past_isfq[i] *= 0.5 */
         past_isfq[i] = (Word16t)((isf_q[i] - L_tmp) >> 1);
      }
   }
                           ...
                           Word16t *isfold, Word16t *isf_buf, Word16t bfi)
{
   ...
   if(bfi == 0) /* Good frame */
   {
      ...
      for(i = 0; i < 3; i++)
      {
         isf_q[i] = (Word16t)(isf_q[i] + D_ROM_dico21_isf[indice[2] * 3 + i]);
      }

      for(i = 0; i < 3; i++)
      {
         isf_q[i + 3] =
            (Word16t)(isf_q[i + 3] + D_ROM_dico22_isf[indice[3] * 3 + i]);
      }

      for(i = 0; i < 3; i++)
      {
         isf_q[i + 6] =
            (Word16t)(isf_q[i + 6] + D_ROM_dico23_isf[indice[4] * 3 + i]);
      }

      for(i = 0; i < 3; i++)
      {
         isf_q[i + 9] =
            (Word16t)(isf_q[i + 9] + D_ROM_dico24_isf[indice[5] * 3 + i]);
      }

      for(i = 0; i < 4; i++)
      {
         isf_q[i + 12] =
            (Word16t)(isf_q[i + 12] + D_ROM_dico25_isf[indice[6] * 4 + i]);
      }
...
void D_LPC_isf_extrapolation(Word16t HfIsf[])
{
   ...
   exp2 = D_UTIL_norm_s((Word16t)tmp2);
   exp = D_UTIL_norm_s((Word16t)tmp);
   exp = exp - 1;
   tmp = tmp << exp;
   tmp2 = tmp2 << exp2;
   ...
   for(i = M - 1; i < M16k - 1; i++)
   {
      HfIsf[i] = (Word16t)(HfIsf[i - 1] + IsfDiff[i - (M - 1)]);
   }

   for(i = 0; i < M16k - 1; i++)
   {
      HfIsf[i] = (Word16t)((HfIsf[i] * 13107) >> 14);
