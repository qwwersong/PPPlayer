 ...
 */
int D_DTX_reset(D_DTX_State *st, const Word16t *isf_init)
{
   Word32 i;
   ...
   memcpy(st->mem_isf, isf_init, M * sizeof(Word16t));
   memcpy(st->mem_isf_prev, isf_init, M * sizeof(Word16t));

   for(i = 0; i < D_DTX_HIST_SIZE; i++)
   {
      memcpy(&st->mem_isf_buf[i * M], isf_init, M * sizeof(Word16t));
 ...
 *    non-zero with error, zero for ok
 */
int D_DTX_init(D_DTX_State **st, const Word16t *isf_init)
{
   D_DTX_State *s;
 ...
 *    void
 */
static void D_DTX_cn_dithering(Word16t isf[M], Word32 *L_log_en_int,
                               Word16t *dither_seed)
{
   ...
   *L_log_en_int = *L_log_en_int + ((rand_dith * GAIN_FACTOR) << 1);
   ...
   for(i = 1; i < M - 1; i++)
   {
      ...
      {
         isf[i] = (Word16t)(isf[i - 1] + ISF_DITH_GAP);
      }
      else
      {
         isf[i] = (Word16t)temp;
 ...
 *    void
 */
void D_DTX_exe(D_DTX_State *st, Word16t *exc2, Word16t new_state, Word16t isf[],
               Word16t **prms)
{
   ...
   Word16t log_en_index;
   Word16t tmp_int_length;
   Word16t exp, log_en_int_e, log_en_int_m, level;
   ...
   if((st->mem_dtx_hangover_added != 0) & (st->mem_sid_frame != 0))
   {
      ...
      memcpy(&st->mem_isf_buf[ptr * M], &st->mem_isf_buf[st->mem_hist_ptr * M], M * sizeof(Word16t));
      ...
      for(i = 0; i < D_DTX_HIST_SIZE; i++)
      {
          ...
          * is in Q10
          */
         st->mem_log_en = (Word16t)(st->mem_log_en + st->mem_log_en_buf[i]);
      ...
      for(j = 0; j < M; j++)
      {
         st->mem_isf[j] = (Word16t)(L_isf[j]>>3);   /* divide by 8 */
      }
   }
   ...
   if(st->mem_sid_frame != 0)
   {
       ...
       */
      memcpy(st->mem_isf_prev, st->mem_isf, M * sizeof(Word16t));
      ...
      if(st->mem_valid_data != 0) /* new data available (no CRC) */
      {
         ...
         if(tmp_int_length >= 2)
         {
            st->mem_true_sid_period_inv =
               (Word16t)(0x2000000 / (tmp_int_length << 10));
         }
         else
         {
          ...
          */
         st->mem_log_en = (Word16t)(log_en_index << (15 - 6));

         /* Divide by 2.625  */
         st->mem_log_en = (Word16t)((st->mem_log_en * 12483) >> 15);
   ...
   {
      /* Q14 * Q15 -> Q14 */
      isf[i] = (Word16t)((int_fac * st->mem_isf[i]) >> 15);
   }
   ...
   /* Find integer part  */
   log_en_int_e = (Word16t)((L_log_en_int)>>16);

   /* Find fractional part */
   log_en_int_m = (Word16t)((L_log_en_int - (log_en_int_e << 16)) >> 1);
    ...
    */
   log_en_int_e = (Word16t)(log_en_int_e + (16 - 1));
   ...
   exp0 = (15 - exp0);
   level = (Word16t)(level32 >> 16);   /* level in Q15 */

   /* generate white noise vector */
   for(i = 0; i < L_FRAME; i++)
   {
      exc2[i] = (Word16t)((D_UTIL_random(&(st->mem_cng_seed)) >> 4));
   }
 ...
 *    void
 */
void D_DTX_activity_update(D_DTX_State *st, Word16t isf[], Word16t exc[])
{

   Word32 L_frame_en, log_en;
   Word32 i;
   Word16t log_en_e, log_en_m;

   st->mem_hist_ptr = (Word16t)(st->mem_hist_ptr + 1);
   ...
   memcpy(&st->mem_isf_buf[st->mem_hist_ptr * M], isf, M * sizeof(Word16t));
   ...
   /*
    * convert exponent and mantissa to Word16t Q7.
   ...
   st->mem_log_en_buf[st->mem_hist_ptr] = (Word16t)log_en;
